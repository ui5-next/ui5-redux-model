...
export default class ReduxModel<T> extends ClientModel {
   ...
   * @param {boolean} persist persist data
   */
  constructor(initializeState: T = {}, persist = false) {

    super();

    this.setSizeLimit(DEFAULT_MODEL_LIST_MAX_SIZE);

    this._initStore(initializeState, persist);

  }

  _initStore(initState: T, persist: Boolean) {
    ...
    this._baseReducer = (oState = initState, oActionData: ActionData) => {
    ...
    if (persist) {
      ...
      this._store = createStore(
        persistReducer({ key: "GlobalState", storage }, this._baseReducer),
